...
export class AuthService {
  ...
  login(email: string, password: string): Observable<void> {
    return this.http.post<LoginResponse>('http://192.168.10.87:7108/api/v1/Auth/login', { email, password }).pipe(
      switchMap(response => {
  ...
  getUser(userId: string): Observable<UserResponse> {
    ...
    return this.http.get<UserResponse>(`http://192.168.10.87:7108/api/v1/users/${userId}`, { headers });
  }
